...
int klibc_vsnprintf(char *buffer, size_t sz, char const *format, va_list ap)
{
    struct __snprintf_core core;
    int exit;
...
int klibc_snprintf(char *buffer, size_t sz, char const *format, ...)
{
    va_list ap;
    int n;

    va_start(ap, format);
    n = klibc_vsnprintf(buffer, sz, format, ap);
    va_end(ap);
    return n;
}
